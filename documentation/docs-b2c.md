...
1. [AML & APEX Renderer](#aml--apex-renderer)
...
## 5.1 AML Specification (Adaptive Markup Language)

AML is the "script" that drives the APEX Player. In the B2C context, these scripts are dynamically generated by grounding the LLM in the student's uploaded course materials.
...
## 5.2 The APEX Renderer Engine (Next.js/Canvas)

The renderer is a client-side engine that interprets AML and draws it to a `<canvas>` element.
...
export class APEXRenderer {
  ...
  private renderLoop(): void {
    ...
    if (this.currentSequence) {
      const activeActions = this.currentSequence.actions.filter(a =>
        this.parseTime(a.at) <= elapsedTime
      );
      activeActions.forEach(action => this.renderAction(action));
    }

    requestAnimationFrame(() => this.renderLoop());
  }

  private renderAction(action: AMLAction): void {
    // Logic for rendering instructor avatar, text, and shapes
    // For B2C, this includes LaTeX rendering for math equations
  }

  private parseTime(at: number | string): number {
    return typeof at === 'number' ? at : parseFloat(at.replace('+', ''));
  }

  private drawBackground() { /* ... gradents/styles ... */ }
}
```

## 5.3 Application-Level Flow (B2C Context)

In the B2C "Personal Study Companion", AML generation and rendering work as follows:

1.  **Context-Aware Generation**: When a student enters a "Concept Block", a Convex Action queries the **Vector Index** for the relevant chunks of the student's PDF.
2.  **AML Prompting**: The LLM (Gemini or DeepSeek) is given these chunks and instructed to "Explain these concepts using the following AML format".
3.  **Adaptive Interrupts**: If a student clicks "Ask Question" mid-lesson, the player pauses. The question is sent to Convex, which generates a *correction sequence* in AML. This new sequence is injected into the player's queue immediately.
4.  **State Sync**: Every time a sequence finish or a checkpoint is passed, the frontend calls a Convex mutation (`updateXp` or `completeModule`) to sync the student's progress and update their **Study Streak**.
